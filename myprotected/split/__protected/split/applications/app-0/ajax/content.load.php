<?php 
	...
	$uncard_data = unserialize($card_data);
 ?>
...
<?php
	...
	if($tables[0]['where'] != null)
	{
		$is_where = true;
		$rule_where = " AND ".$tables[0]['where'];
	}
	...
	if($is_where) $where_sql .= $rule_where;
	...
	//$query = "SELECT COUNT(*) FROM [pre]users WHERE 1 ORDER BY id";
			...
			$items_stmt	= $dbh->prepare($quant_tmp_query);
			$items_arr	= $items_stmt->execute();
			$items		= $items_arr->fetchallAssoc();
					
	$pages = ceil($items[0]['COUNT(*)']/$on_page);
                 	...
                 	<?php
                  	foreach($fields as $field)
				  	{
						switch($field['type'])
						{
							case 'sql_query':
							{
								if($_POST['filtr_table'] != 'users_types')
								{
									$query = $field['query'];
									
									$_stmt = $dbh->prepare($query);
									$_arr = $_stmt->execute($row[$field['ref_field']]);
									
									$_res = $_arr->fetchallAssoc();
								}else
								{
									$query = "SELECT ".$field['value']." FROM [pre]".$field['table']." WHERE `".$field['ref_id']."`='".$row['id']."' LIMIT 1";
								...
								?>
								<td><?php echo $_res[0][$field['value']]; ?></td>
								<?php
								break;
							}
							case 'text':
							...
							{
								?>
								<td><?php echo mb_substr($row[$field['value']],0,$field['size']); ?></td>
								<?php
								break;
							}
							case 'date':
							...
							{
								$link_name = "";
								if($uncard_data['type'] != null && $uncard_data['type'] == 'dialog' && $row['from_id'] != null && $row['from_id'] == ADMIN_ID)
								{
									$query = "SELECT name, fname FROM [pre]users WHERE `id`='".$row['to_id']."' LIMIT 1";
									...
									$link_name = $dialog_user['name']." ".$dialog_user['fname'];
								}else
								{
                        			...
                        			<div>
                                    	<a  href="javascript:void(0);" 
                                        	onclick="
													...
													// Вызов карточки элемента из списка
													?>" 
							...
							case 'sql_query':
							{
								...
								?><td><?php echo $_row[$field['value']] ?></td><?php
								break;
							}
							default:
							{
...
<?php
	...
	{
		if($uncard_data['type'] == 'dialog' || true)
		{
			?>
				<script src="split/js/iload.js" type="text/javascript"></script>
			<?php
		}
?>
...
<script type="text/javascript" language="javascript">
	...
	var card_data = '<?php echo $card_data ?>';
	var app_table = '<?php echo $uncard_data['table'] ?>';
	
	$(function(){
			global_card_data = card_data;
		});
