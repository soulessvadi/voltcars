...
<?php
	...
	$query = "SELECT COUNT(*) FROM [pre]shop_catalog WHERE 1 ORDER BY id";
	...
	$query = "SELECT * FROM [pre]shop_catalog WHERE 1 ORDER BY order_id LIMIT ".($start_page-1)*$on_page.",".$on_page;
	...
	foreach($data as $item_num => $item_val)
	{
		...
		if($item_val['block'] == "0")
		{
			...
			$data[$item_num]['block'] = "Опубликована";
		}else
		{
			$data[$item_num]['publish_class'] = "not-published";
			$data[$item_num]['block'] = "Не опубликована";
		}
			...
			<div class="r-z-c-table">
            	<table class="maintable" id="main-table">
                    <div class="head-tr">
                        ...
                        <th class="main-t-th" style="">Алиас</th>
				...
				<?php
                	foreach($data as $row)
					{
						...
						$query = "	SELECT prod_id FROM [pre]shop_cat_prod_ref WHERE cat_id=".$row['id'];
