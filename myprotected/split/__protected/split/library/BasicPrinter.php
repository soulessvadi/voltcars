...
class BasicPrinter
{
		...
		public function print_autocomplete($title,$name,$hold,$id,$size=25,$value)
		{
			...
			<div class='zen-form-item'>
				...
				<div class='zif-wrap'>
                	<input	id='autocomplete-$name' class='my-field' type='$input_type' placeholder='$hold'  
                    		value='$value' name='$name' size='$size' />
                </div>
            </div>
		...
		public function print_image_mult($header, $field, $value, $path, $appTable ,$id, $mas_field)
		{
			$result = "
			 <div class='clear'></div>
        	
            <input type='file' multiple name='".$field."[]' class='hidden' id='editor-file-input' onChange=\"split_txt($(this).val());\">
		...
		foreach($value as $val_item)
		{
            if(file_exists("../../../../..".$path.$val_item[$mas_field]) && trim($val_item[$mas_field]) != "")
			{
				...
				<div class='item' id='media-item-".$val_item['id']."'>
                	<img class='close-file-item' alt='X' title='Удалить файл' src='".WP_FOLDER."img/close-icon.png' onClick=\"delete_media_item_ref('".$val_item['id']."','$path');\">
					<div class='item-inside'>
                    	<a class='theater' href='".$path.$val_item[$mas_field]."' title='Modal view' rel='group'>
							<img alt='File not found' src='".$path.$val_item[$mas_field]."'>
						</a>
                    <div class='icode hidden' contenteditable='true' 
                    	onmouseover=\"$(this).stop().animate({'bottom':'0px'},400,'easeInOutExpo');\"
                        onmouseout=\"$(this).stop().animate({'bottom':'-80px'},400,'easeInOutExpo');\">
		...
		public function print_shopProductChars($header,$field,$chars)
		{
			$result = "";
			$result .= "
			...
			<br>
				...
				foreach($chars as $char)
				{
                	...
                	<tr>
                        ...
                        <td>
                        	<input id='char-".$char['id']."' class='my-field' type='text' placeholder='".$char['id']."' value='".$char['value']."' name='".$field."[".$char['id']."]' size='25' maxlength='100'>
						</td>
					</tr>
					";
				}
