<?php // ajax json action
	...
	if($catId > 0)
	{
		...
		if($productsMassive)
		{
			...
			$result = $ah->print_adding_products_for_banner($productsMassive,$prodId,"add_banner_prod_from_modal");
		}else
		{
	...
	}elseif(isset($_POST['key']) && $_POST['key']!='')
	{
		...
		$query = "SELECT M.* , 
					...
					(SELECT file FROM [pre]files_ref WHERE `ref_table`='shop_products' AND `ref_id`=M.id AND `file`!='fail' AND `file`!='0' AND `file`!='' AND `file`!='NULL' LIMIT 1) as file 
