...
<?php
	...
	$query = "SELECT COUNT(*) FROM [pre]stocks WHERE 1 ORDER BY id";
	...
	$query = "SELECT * FROM [pre]stocks WHERE 1 ORDER BY order_id LIMIT ".($start_page-1)*$on_page.",".$on_page;
				...
				?>
                	<tr class="<?php echo $row['row_class'] ?>" id="<?php echo $row['id'] ?>">
                        ...
                        <td class="main-t-td-name">
                        	<div><a href="javascript:void(0);" onclick="change_head(3); load_card(card_path,<?php echo $row['id'] ?>);"><?php echo $row['name'] ?></a></div>
                        </td>
