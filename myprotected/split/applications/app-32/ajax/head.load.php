 ...
 ?>
            ...
            <div class="r-z-h-saving">
                ...
                <button class="r-z-h-s-delete nonactive first-actives" type="button" title="Опция не активна, для активации необходимо отметить елементы из списка." 
                		id="delete-checked-button" onclick="show_is_delete_stock_fields();"></button>
                
                <button class="r-z-h-s-create" type="button" onclick="change_head(2);  load_create(create_path);" title="Создать элемент склада">Создать &nbsp;&nbsp;&nbsp;<span>+</span></button>
            </div><!-- r-z-h-saving -->	
			</div>
...
<script type="text/javascript" language="javascript">
	...
	function show_is_delete_stock_fields()
	{
		var is_nonactive = $('#delete-checked-button').attr('class');
		if(is_nonactive != 'r-z-h-s-delete nonactive')
		{
			var data = {'answer_path':'<?php echo WP_FOLDER.'ajax/modal/stocks/answer.delete.stock.fields.php' ?>'}
			open_modal('<?php echo WP_FOLDER.'ajax/modal/stocks/question.delete.stock.fields.php' ?>',data);
		}
	}
	function activate_stocks()
	{
		...
		if(is_nonactive != 'r-z-h-s-save nonactive')
		{
			...
			$('#modal-window').load('<?php echo WP_FOLDER.'ajax/modal/stocks/action.activate.stock.fields.php' ?>',data);
		}
	}
	function disactivate_stocks()
	{
		...
		{
			var data = { 'list[]':cur_list }
			$('#modal-window').load('<?php echo WP_FOLDER.'ajax/modal/stocks/action.disactivate.stock.fields.php' ?>',data);
		}
	}
