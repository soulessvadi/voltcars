...
class ajaxHelp
{
		...
		public function __construct($dbh)
		{
			...
			$siteSettingsData = $this->rs("SELECT * FROM [pre]total_config WHERE `id`=1 LIMIT 1");
			$this->siteSettings = ($siteSettingsData ? $siteSettingsData[0] : array());
			
			// Стягиваем настройки магазина
			
			$shopSettingsData = $this->rs("SELECT * FROM [pre]shop_settings WHERE `id`=1 LIMIT 1");
			$this->shopSettings = ($shopSettingsData ? $shopSettingsData[0] : array());
			
			$this->USD_EX = $this->shopSettings['usd_ex'];
			$this->EUR_EX = $this->shopSettings['eur_ex'];
		} 
	...
	*/
	...
	public function is_present($prod_id, $quant=1)
	{
		$query = "
				SELECT B.id, B.nominal, R.id as banner_ref_id  
					FROM 
					[pre]banners as B 
					LEFT JOIN [pre]shop_banner_prod_ref as R on R.banner_id = B.id 
					WHERE R.prod_id = $prod_id AND B.pos_id=4 
					ORDER BY B.id DESC 
					LIMIT 1
				";
		$result = $this->rs($query);
		...
		if($result)
		{
			...
			$banner_ref_id = $result[0]['banner_ref_id'];
			
			$query = "
				SELECT P.id, P.name, P.alias, P.price,
					(SELECT file FROM [pre]files_ref WHERE `ref_table`='shop_products' AND `ref_id`=P.id AND `file`!='' ORDER BY id LIMIT 1) as filename     
					FROM 
					[pre]shop_products as P 
					LEFT JOIN [pre]shop_banner_prod_access_ref as R on R.prod_id = P.id 
					WHERE R.banner_ref_id = $banner_ref_id 
					ORDER BY P.id
					LIMIT 10
				";
			$result[0]['items'] = $this->rs($query);
		}else
		{
			$sub_query = "SELECT M.*,  M.acc_id as id , R.cat_id,
					 
					 (SELECT name FROM [pre]shop_products WHERE `id`=M.acc_id LIMIT 1) as name,
					 (SELECT sku FROM [pre]shop_products WHERE `id`=M.acc_id LIMIT 1) as sku,
					 (SELECT price FROM [pre]shop_products WHERE `id`=M.acc_id LIMIT 1) as price,
					 (SELECT sale_price FROM [pre]shop_products WHERE `id`=M.acc_id LIMIT 1) as sale_price,
					 (SELECT alias FROM [pre]shop_products WHERE `id`=M.acc_id LIMIT 1) as alias,
					 
					 (SELECT file FROM [pre]files_ref WHERE `ref_table`='shop_products' AND `ref_id`=M.acc_id LIMIT 1) as filename 
					 
						FROM [pre]shop_prod_present_ref as M 
						
						JOIN [pre]shop_cat_prod_ref as R ON R.prod_id=M.acc_id
						  
						WHERE M.prod_id=$prod_id 
						LIMIT 10";
			$items = $this->rs($sub_query);
	...
	public function is_sertificate($prod_id, $quant=1)
	{
		$query = "
				SELECT B.id, B.nominal  
					...
					[pre]banners as B 
					LEFT JOIN [pre]shop_banner_prod_ref as R on R.banner_id = B.id 
					WHERE R.prod_id = $prod_id AND B.pos_id=1 
					ORDER BY B.id DESC 
					LIMIT 1
				";
		$result = $this->rs($query);
	...
	public function getActualPrice($arr)
    {
        $price = ( !$arr['sale_price'] ?  $arr['price'] :  $arr['sale_price'] );
		$old_price = ( $arr['sale_price'] ?  $arr['price'] :  0 );
                                        
        if(!$price)
        {
            if($arr['usd_price'])
            {
                $price = $arr['usd_price']*$this->USD_EX;
            }
            elseif($arr['eur_price'])
            {
                $price = $arr['eur_price']*$this->EUR_EX;
            }
        }
	...
	public function getUserCartTableHtml($_order_id, $_path_to_cart_table_view="split/view_parts/inc/_cart_table.php", $is_ajax=true, $action="", $_fa="cart")
	{
		...
		if($is_ajax)
		{
			switch($action)
			{
				...
				case "add_complect":
				{
					...
					if(isset($_POST['complect_id']) && $_POST['complect_id'])
					{
						$complect_id = (int)$_POST['complect_id'];
						$complect_data = $this->rs("SELECT * FROM [pre]shop_prod_complect_ref WHERE `id`='".$complect_id."' LIMIT 1");
						
						$post_quant = 1;
                        
                        if($complect_data)
						{
                            $complect_data = $complect_data[0];
                            
                            $prod_id = $complect_data['prod_id'];
						    $prod_data = $this->rs("SELECT price FROM [pre]shop_products WHERE `id`='".$prod_id."' AND `block`=0 LIMIT 1");
                            
                            $acc_id = $complect_data['acc_id'];
                            $c_price = $complect_data['c_price'];
                            $acc_data = $this->rs("SELECT price FROM [pre]shop_products WHERE `id`='".$acc_id."' AND `block`=0 LIMIT 1");
                            
                            if($prod_data && $acc_data)
                            {
                                $get_query = "SELECT * FROM [pre]shop_cart_orders WHERE `complect_id`='".$complect_id."' AND `order_id`='".$_order_id."' LIMIT 1";
                                $cart_product_data = $this->rs($get_query);
                                
                                //$test['cart_product_data'] = $cart_product_data;
                                
                                if(!$cart_product_data)
                                {
                                    $query = "INSERT INTO [pre]shop_cart_orders (`order_id`,`complect_id`,`quant`,`dateCreate`,`dateModify`)
                                            VALUES
                                            ('".$_order_id."','".$complect_id."','".$post_quant."','".$curr_date."','".$curr_date."')";
                                    self::rss($query);
        ...
        {
            $cart_data[$i]['complect'] = array();
            
            if($cart_item['complect_id'])
            {
               $complect_id = $cart_item['complect_id'];
                
               $complect_data = $this->rs("SELECT M.*, 
                                            (SELECT name FROM [pre]shop_products WHERE id=M.prod_id) as prod_name,
                                            (SELECT alias FROM [pre]shop_products WHERE id=M.prod_id) as prod_alias,
                                            (SELECT price FROM [pre]shop_products WHERE id=M.prod_id) as prod_price,
                                            (SELECT file FROM [pre]files_ref WHERE `ref_table`='shop_products' AND `ref_id`=M.prod_id AND `file`!='0' AND `file`!='' LIMIT 1) as prod_file,
                                            
                                            (SELECT name FROM [pre]shop_products WHERE id=M.acc_id) as acc_name,
                                            (SELECT alias FROM [pre]shop_products WHERE id=M.acc_id) as acc_alias,
                                            (SELECT price FROM [pre]shop_products WHERE id=M.acc_id) as acc_price,
                                            (SELECT file FROM [pre]files_ref WHERE `ref_table`='shop_products' AND `ref_id`=M.acc_id AND `file`!='0' AND `file`!='' LIMIT 1) as acc_file
               ...
               if($complect_data)
               {
                   $cart_data[$i]['complect'] = $complect_data[0];
               }
            }
        }
		...
		foreach($cart_data as $i => $cart_item)
		{
			...
			// Present
                $this_is_present = $this->is_present($curr_prod_id, $curr_prod_quant);
                
                $cart_data[$i]['present'] = $this_is_present;
                ...
                // Accessuare
				if(!$this_is_present && !$this_is_sertificate)
				{
					$this_is_accessuare = true; // $this->is_accessuare($curr_prod_id);
				}else
				{
					$this_is_accessuare = false;
				}
		...
		//=========================================================================================
			...
			foreach($cart_data as $i => $cart_item)
				{
					$curr_prod_id = $cart_item['prod_id'];
					$curr_complect_id = $cart_item['complect_id'];
					
					$cart_product_data = $this->rs("SELECT M.id, M.name, M.alias, M.price, M.sale_price, M.usd_price, M.eur_price, M.in_stock,  
													(SELECT file FROM [pre]files_ref WHERE `ref_table`='shop_products' AND `ref_id`=M.id ORDER BY id LIMIT 1) as filename 
													FROM [pre]shop_products as M 
					...
					if($cart_product_data)
					{
                        ...
                        $curr_cart_product['aprice'] = $this->getActualPrice($curr_cart_product);
						
						//echo "<pre>"; print_r($curr_cart_product['aprice']); echo "</pre>";
						...
						$curr_cart_product['present'] = $cart_item['present'];
			...
			if($cart_data)
			{
				foreach($cart_data as $cart_item)
				{
					...
					if($curr_prod_id)
					{
						$curr_cart_product = $currProducts[$curr_prod_id];
						
						$cart_data_product[$cart_item['id']] = $curr_cart_product; // info for cart page
						
						$cart_total_quant += $cart_item['quant'];
						$cart_total_summ += ($curr_cart_product['aprice']['price']*$cart_item['quant']);
						
						//echo $cart_total_summ . " | ";
					
					}elseif($curr_complect_id)
					{	
                        //echo "<p>+ ".(($cart_item['complect']['prod_price']+$cart_item['complect']['c_price'])*$cart_item['quant'])."</p>";
                        
						$cart_total_quant += $cart_item['quant']*2;
						$cart_total_summ += (($cart_item['complect']['prod_price']+$cart_item['complect']['c_price'])*$cart_item['quant']);
					}
				}
			}
