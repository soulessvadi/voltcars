					...
					switch($columnParams['type'])
					{
						...
						case 'show_gal_img':{
							$result .= "<td class='main-t-td-name'>
											<img src='../split/files/images/gallery/".$item[$columnParams['field']]."' alt='Galeery image' style='width:100px; height: auto;'/>
                            			</td>";
							break;
						}
						case 'block':
							...
							if(isset($columnParams['params']))
							{
								if(isset($columnParams['params']['type']))
								{
									switch($columnParams['params']['type'])
									{
										...
										case 'test_drive_view':
										{
											$result .= "<td class='main-t-td-name'>
											<a class='view-link'	href='javascript:void(0);' onclick=\"loadPage('personal','test-drive-orders',46,$iid,'cardView',{}); update_viewed_status($iid);\"><img title='Детальный просмотр' src='split/img/glaz.png'></a>
                            				</td>";
											break;
											// $columnParams['field']
										}
										case 'contact_message_view':
										{
											$result .= "<td class='main-t-td-name'>
											<a class='view-link'	href='javascript:void(0);' onclick=\"loadPage('personal','contact-form',46,$iid,'cardView',{}); contact_message_view($iid);\"><img title='Детальный просмотр' src='split/img/glaz.png'></a>
                            				</td>";
											break;
											// $columnParams['field']
										}
		...
		public function getCardViewTable($params=array())
		{
			...
			foreach($params['cardTmp'] as $header => $tmp)
			{
				...
				switch($tmp['type'])
				{
					case 'image':
					{
						$filePath = ($_SERVER['SERVER_NAME']=='localhost'
										?
										".." . $tmp['params']['path'].$params['cardItem'][$tmp['field']]
										:
										$tmp['params']['path'].$params['cardItem'][$tmp['field']]
									);

						$result .= "<td>".( (file_exists(str_replace("....","..",$params['rootPath'].$filePath)) && trim($params['cardItem'][$tmp['field']]) != "" ) ? "
					...
					case 'style-text':
					{
						$result .= "<td>".$this->printStyleText($params['cardItem'][$tmp['field']])."</td>";
						break;
					}
					case 'arr_mono':
					{
		...
		public function getCardEditForm($params=array())
		{
			...
			foreach($params['cardTmp'] as $fieldHeader => $tmp)
			{
				...
				switch($tmp['type'])
				{
					...
					case 'code_html':
					{
						$result .= $this->print_code_html($fieldHeader, $tmp['field'], $item[$tmp['field']]);
						break;
					}
					case 'date':
					{
					...
					case 'image_mono':
					{
						$option_delete = (isset($tmp['params']['option_delete']) ? $tmp['params']['option_delete'] : true);
						
						$result .= $this->print_image_mono($fieldHeader, $tmp['field'], $item[$tmp['field']], $tmp['params']['path'], $tmp['params']['appTable'], $tmp['params']['id'], $option_delete);
