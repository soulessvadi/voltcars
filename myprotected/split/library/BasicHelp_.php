		...
		public function getCardEditForm($params=array())
		{
			...
			foreach($params['cardTmp'] as $fieldHeader => $tmp)
			{
				switch($tmp['type'])
				{
					...
					case 'hidden':
					{
						$result .= $this->print_hidden($tmp['field'], $item[$tmp['params']['field']]);
						break;
					}
					case 'input':
					{
					...
					case 'select':
					{
						...
						$result .= $this->print_select($fieldHeader, $currValue, $tmp['params']['list'], $tmp['params']['fieldValue'], $tmp['params']['fieldTitle'], $tmp['field'], $tmp['params']['onChange'], $tmp['params']['first']); 
						break;
					}
					case 'redactor':
					{
