...
class BasicHelp extends BasicPrinter
{
		...
		public function getLandingHeader($params=array())
		{
			...
			{
				$result .= "<button class='rzh r-z-h-s-create' type='button' onclick=\"export_products_to_excel();\" title='Export to Excel'>Export Excel</button>";
				
				$result .= "<button class='rzh r-z-h-s-create' type='button' onclick=\"export_products_to_hotline_xml();\" title='HotLine Export'>Hotline Export</button>";
			}
				...
				switch($params['type'])
				{
					...
					case 'shopManage':
					{
						...
						$result .= "
				    			...
				    			<button class='rzh r-z-h-s-create' type='button' onclick=\"export_products_to_excel();\" title='Export to Excel'>Export all products</button>
								...
								<button class='rzh r-z-h-s-create' type='button' onclick=\"$('#check_import_file').click();\" title='Import from Excel'>Import updates</button>
								...
								<button class='rzh r-z-h-s-create' type='button' onclick=\"open_new_modal('importClimatExcel',{});\" title='Импортировать обновления по Климату с Excel'>Климат Excel импорт &nbsp;&nbsp;&nbsp;<span></span></button>
								";
                        ...
                        <div class='r-z-c-f-fil-select-item'>
                            ...
                            <div class='styled-select'>                	
                                <select id='r-z-c-f-sort-amount' name='quant_filtr' onchange=\"$.cookie('global_on_page',$(this).val()); 
										loadPage('".$params['headParams']['parent']."','".$params['headParams']['alias']."',".$params['headParams']['id'].",0,'landingPage',{start:'1',filtr:$('#wp-filtr-form').serialize()});\">
                                    <option ".($params['on_page'] == 1 ? "selected" : "")." data-skip='1' value='1'>1</option>
									<option ".($params['on_page'] == 5 ? "selected" : "")." data-skip='1' value='5'>5</option>
									...
									<option ".($params['on_page'] == 150 ? "selected" : "")." data-skip='1' value='150'>150</option>
                                </select><!-- r-z-c-f-fil-category -->
                            </div>
			...
			foreach($params['itemsList'] as $item)
			{
				...
				foreach($params['tableColumns'] as $columnHeader => $columnParams)
				{
					switch($columnParams['type'])
					{
						...
						case 'image':
						{
							//$result .= "<td class='check-col'>image</td>";
							
							$image_path = $columnParams['params']['path'].$item[$columnParams['field']];
							$image_full_path = $columnParams['params']['full_path'].$item[$columnParams['field']];
							
							$result .= "<td class='image'><a class='theater' href='javascript:void(0);' onclick=\"loadPage('".$params['headParams']['parent']."','".$params['headParams']['alias']."',".$params['headParams']['id'].",$iid,'cardEdit',{});\">
								<img alt='БЕЗ КАРТИНКИ' style='max-width:100px; height:auto;' src='$image_path'></a>
								</td>";
							break;
						}
						case 'update_cat_pos':
						...
						case 'block':
						{
							$result .= "<td class='publication'>
                            				...
                            				<span>".(!$item[$columnParams['field']] ? "Да" : "Нет")."</span>
										</td>";
							break;
						}
						case 'yesno':
						{
							$result .= "<td class='yes_or_no'>
											<div class='".($item[$columnParams['field']] ? "published" : "not-published")."' style='margin-right:5px;'></div>
                            				<span>".($item[$columnParams['field']] ? " Да" : " Нет")."</span>
										</td>";
							break;
						}
				...
				switch($tmp['type'])
				{
					...
					case 'images':
					{
						...
						foreach($params['cardItem'][$tmp['field']] as $currImg)
						{
							...
							{
								$path_to_file_crop  = $tmp['params']['path']."crop/320x240_".$currImg[$tmp['params']['field']];
							}
		...
		public function getCardEditForm($params=array())
		{
			...
			foreach($params['cardTmp'] as $fieldHeader => $tmp)
			{
				switch($tmp['type'])
				{
					...
					case 'banner_access_script':
					{
						$result .= $this->clear();
						$result .= $this->banner_access_script($item_id,$tmp['access_list']);
						break;
					}
					case 'banner_prod_script':
					{
						$result .= $this->clear();
						$result .= $this->banner_prod_script($item_id,$tmp['prod_list']);
						break;
					}
					...
					case 'prod_present_script':
					{
						$result .= $this->clear();
						$result .= $this->prod_present_script($item_id,$tmp['access_list']);
						break;
					}
					case 'header':
					{
					...
					case 'area_id':
					{
						...
						$result .= "<div class='area_id' id='".$tmp['field']."'></div>";
						break;
					}
					case 'cartProducts':
					{
						...
						//echo "<pre>"; print_r($tmp['params']['items']); echo "</pre>";
