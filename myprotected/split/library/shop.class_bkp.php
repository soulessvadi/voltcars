...
class settingsHelp extends BasicHelp
{
			...
			if($result)
			{
				...
				if($result['charsGroup'])
				{
					...
					$query = "SELECT M.id as char_id, M.measure, M.title, M.name, M.pos, 
							  ...
							  (SELECT value2 		FROM [pre]shop_chars_prod_ref_v2 WHERE char_id=M.id AND prod_id=$id) as value2
							  
							 FROM  
							 	[pre]shop_chars as M  
							 WHERE M.group_id='".$cat_chars_group_id."'
							 ORDER BY M.pos
							 LIMIT 100";
					/*
					...
					*/
					...
					{
						$query = "SELECT * FROM [pre]shop_chars WHERE `group_id`=$cat_chars_group_id ORDER BY pos LIMIT 100";
						$charsList = $this->rs($query);
					}
		...
		public function getAllShopProducts($params = array())
		{
			...
			if(isset($params['filtr']['massive']))
			{
				foreach($params['filtr']['massive'] as $f_name => $f_value)
				{
					if($f_name=='G.group_id') $G_group_id = true;
			...
			$G_group_join = ($G_group_id ? ' INNER JOIN [pre]shop_prod_group_ref AS G on G.prod_id=M.id  ' : '');
			...
			// INNER JOIN [pre]shop_catalog AS CP on CP.id = C.parent 
			
			$query = "SELECT SQL_CALC_FOUND_ROWS M.id, M.hotline, M.name, M.sku, M.price, M.block, C.name as cat_name,
			
						(SELECT file FROM [pre]files_ref  WHERE `ref_table`='shop_products' AND `ref_id`=M.id LIMIT 1) as product_image,
						(SELECT name FROM [pre]users  WHERE id = M.adminMod LIMIT 1) as admin_name,
						(SELECT fname FROM [pre]users  WHERE id = M.adminMod LIMIT 1) as admin_fname 
			...
			//echo "<pre>QUERY: ".str_replace("[pre]","next_",$query)."</pre>";
		...
		public function getShopOrders($params=array(),$typeCount=false)
		{
			...
			if(!$typeCount)
			{
				...
				$query = "SELECT M.id, M.quick, M.author_id, M.user_id, M.paid_status, M.pay_method, M.delivery_method, M.products_quant, M.sum, M.dateCreate, M.client_name, M.client_fname,  
		...
		{
			$query = "SELECT id,name FROM [pre]shop_products_groups WHERE 1 ORDER BY id LIMIT 1000";
			return $this->rs($query);
		}
		...
		{
			$query = "SELECT M.* ,
							(SELECT DescriptionRu FROM [pre]np_cities WHERE `Ref`=M.np_city_ref LIMIT 1) as np_city_name,
							(SELECT DescriptionRu FROM [pre]np_parts WHERE `Ref`=M.np_part_ref LIMIT 1) as np_part_name,
							(SELECT DescriptionRu FROM [pre]np_cities WHERE `Ref`=M.np_city_ref2 LIMIT 1) as np_city_name2
							FROM [pre]shop_orders as M 
							WHERE M.id='$id' 
							LIMIT 1
							";
		...
		public function getShopOrderView($card,$params=array())
		{
			...
			$str .= "
					<table class='intotal'>
						...
						<tr>
							<td>Новая почта ГОРОД:</td>
							<td>".$card['np_city_name']."</td>
						</tr>
						<tr>
							<td>Новая почта ОТДЕЛЕНИЕ:</td>
							<td>".$card['np_part_name']."</td>
						</tr>
						<tr>
							<td><hr></td>
							<td><hr></td>
						...
						<tr>
							<td>Курьер ГОРОД:</td>
							<td>".$card['np_city_name2']."</td>
						</tr>
						<tr>
							<td>Курьер Улица:</td>
							<td>".$card['add_street']."</td>
						</tr>
						<tr>
							<td>Курьер Дом:</td>
							<td>".$card['add_home']."</td>
			...
			$str .= $this->hr($fieldHeader);
			$str .= "<div class='area_id' id='ProductsCart'></div>";
		...
		public function getProdPresent($id)
		{
			$query = "SELECT M.*,  
					 (SELECT name FROM [pre]shop_products WHERE `id`=M.acc_id LIMIT 1) as acc_name,
					 (SELECT price FROM [pre]shop_products WHERE `id`=M.acc_id LIMIT 1) as acc_price,
