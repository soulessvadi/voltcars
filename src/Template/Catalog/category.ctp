			...
			<span class="alig">
				<ul class="categories_sel">
					<?php foreach ($categories as $key): ?>
						<li><a href="<?= RS."catalog/".$key->alias ; ?>" class="<?= ($key->alias == LA)? 'active' : '' ?>"><?= $key->name; ?></a></li>
						<?php /*<li><button class="<?= ($key->alias == $first_category_alias)? 'active' : '' ?>" type="button" onclick="ajax.load_category(this, '<?= $key->alias; ?>')"><?= $key->name; ?></button></li> */ ?>
					<?php endforeach ?>		
				</ul>
			</span>
